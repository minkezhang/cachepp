...
std::string cachepp::TestResult::to_string(bool is_tsv) {
	...
	buffer << std::setw(pad) << "trial" << sep << std::setw(pad) << "read(%)" << sep << std::setw(pad) << "miss(%)" << sep << std::setw(pad) << "tput(B/ms)" << sep << std::setw(pad) << "lat(ms)" << sep << std::setw(pad) << "line(B)" << sep << std::setw(pad) << "parallel" << sep << std::setw(pad) << "threads" << std::endl;
	buffer << std::string(buffer.str().length(), '=') << std::endl;
	for(size_t index = 0; index < this->size; ++index) {
		buffer << std::setw(pad) << index + 1 << sep << std::setw(pad) << this->get_read_rate(index) << sep << std::setw(pad) << this->get_miss_rate(index)  << sep << std::setw(pad) << this->get_throughput(index)  << sep << std::setw(pad) << this->get_latency(index)  << sep << std::setw(pad) << this->get_line_size(index)  << sep << std::setw(pad) << this->get_is_parallel(index)  << sep << std::setw(pad);
